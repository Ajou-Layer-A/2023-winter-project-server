...
module.exports = (app) => {
   ...
   * @param {*} postData
   */
  route.post('/update', isAuth, async (req, res) => {
    const postData = req.body;
    if (!postData.post_id) {
      return res.status(400).json({
        success: false,
        message: 'Please give me Post Id',
      });
    }
    const user_id = req.decoded.user_id;
    const updatePostResult = await postsController.updatePostData(
      user_id,
      postData
    );
    if (!updatePostResult.success) {
      return res.status(400).json({
        success: false,
        message: 'Update ERR' + updatePostResult.message,
      });
    } else {
      return res.status(200).json({
        success: true,
        message: updatePostResult,
      });
    }
  });
};
